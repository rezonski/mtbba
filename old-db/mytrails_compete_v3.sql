...
--
-- Host: 127.0.0.1
-- Generation Time: May 17, 2016 at 04:21 PM
...
INSERT INTO `repo_regions` (`id`, `id_parent`, `name`, `desc`, `type`, `lat_center`, `lon_center`, `bounds`) VALUES(1, NULL, 'Bosna i Hercegovina', '', 'Country', 0, 0, '');
INSERT INTO `repo_regions` (`id`, `id_parent`, `name`, `desc`, `type`, `lat_center`, `lon_center`, `bounds`) VALUES(2, 1, 'Bjelašnica', '', 'Mountain', 0, 0, '');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `active_trails` AS select `t`.`id` AS `trail_id`,`t`.`id_mnt` AS `mnt_id`,`r`.`name` AS `mnt_name`,`t`.`name` AS `trail_name`,`t`.`desc` AS `trail_desc`,`ty`.`name` AS `type_name`,`ty`.`desc` AS `type_desc`,`v`.`distance` AS `distance`,`v`.`elev_min` AS `elev_min`,`v`.`elev_max` AS `elev_max`,`v`.`elev_gain` AS `elev_gain`,`v`.`elev_loss` AS `elev_loss`,`v`.`review_landscape` AS `review_landscape`,`v`.`review_fun` AS `review_fun`,`v`.`required_fitness` AS `required_fitness`,`v`.`required_technique` AS `required_technique`,`v`.`lat_center` AS `lat_center`,`v`.`lon_center` AS `lon_center`,`v`.`bounds` AS `bounds` from (((`trails` `t` join `trail_versions` `v`) join `repo_regions` `r`) join `repo_types` `ty`) where ((`v`.`id_trail` = `t`.`id`) and (`v`.`active` = 1) and (`t`.`id_mnt` = `r`.`id`) and (`ty`.`id` = `v`.`trail_type`));
