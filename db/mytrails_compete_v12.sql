...
,`trail_name_en` varchar(50)
,`trail_slug` varchar(500)
,`trail_desc` longtext
,`trail_desc_en` varchar(1000)
...
,`point_name_en` varchar(50)
,`point_desc` longtext
,`point_desc_en` varchar(1000)
,`lon` double
,`lat` double
,`elevation` double
,`elevgain` double
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `active_trails`  AS  select `t`.`id` AS `trail_id`,`t`.`name` AS `trail_name`,`t`.`name_en` AS `trail_name_en`,`t`.`slug` AS `trail_slug`,`t`.`desc` AS `trail_desc`,`t`.`desc_en` AS `trail_desc_en`,`ty`.`id` AS `type_id`,`ty`.`name` AS `type_name`,`ty`.`desc` AS `type_desc`,`v`.`distance` AS `distance`,`v`.`elev_min` AS `elev_min`,`v`.`elev_max` AS `elev_max`,`v`.`elev_gain` AS `elev_gain`,`v`.`elev_loss` AS `elev_loss`,`v`.`surface` AS `surface`,`v`.`review_landscape` AS `review_landscape`,`v`.`review_fun` AS `review_fun`,`v`.`required_fitness` AS `required_fitness`,`v`.`required_technique` AS `required_technique`,`v`.`center` AS `center`,`v`.`bounds` AS `bounds`,`v`.`inputfilename` AS `inputfilename`,`v`.`external_link` AS `external_link`,`v`.`image_url` AS `image_url` from ((`trails` `t` join `trail_versions` `v`) join `repo_types` `ty`) where ((`v`.`id_trail` = `t`.`id`) and (`v`.`active` = 1) and (`ty`.`id` = `v`.`trail_type`) and (`ty`.`cat_id` = 0)) ;
DROP TABLE IF EXISTS `active_waypoints`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `active_waypoints`  AS  select `t`.`id` AS `trail_id`,`p`.`id` AS `point_id`,`p`.`name` AS `point_name`,`p`.`name_en` AS `point_name_en`,`p`.`desc` AS `point_desc`,`p`.`desc_en` AS `point_desc_en`,`p`.`lon` AS `lon`,`p`.`lat` AS `lat`,`p`.`elevation` AS `elevation`,`p`.`elevgain` AS `elevgain`,`p`.`elevloss` AS `elevloss`,`p`.`nextelevgain` AS `nextelevgain`,`p`.`nextelevloss` AS `nextelevloss`,`p`.`odometer` AS `odometer`,`p`.`nextstepdist` AS `nextstepdist`,`p`.`symbol` AS `symbol`,`p`.`pictogram` AS `pictogram`,`p`.`pictureurl` AS `pictureurl`,`p`.`time` AS `time`,`p`.`elevationprofile` AS `elevationprofile` from ((`trails` `t` join `trail_versions` `v`) join `trail_version_points` `p`) where ((`v`.`id_trail` = `t`.`id`) and (`v`.`active` = 1) and (`p`.`id_version` = `v`.`id`)) order by `p`.`id` ;
